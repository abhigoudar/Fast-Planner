...
void KinoReplanFSM::init(const rclcpp::Node::SharedPtr nh) {
  ros_node = nh;
  current_wp_  = 0;
  ...
  waypoint_sub_ = ros_node->create_subscription<nav_msgs::msg::Path>("/waypoint_generator/waypoints",
    rclcpp::SystemDefaultsQoS(), std::bind(&KinoReplanFSM::waypointCallback, this, std::placeholders::_1));

  odom_sub_ = ros_node->create_subscription<nav_msgs::msg::Odometry>("/odom_world",
    rclcpp::SystemDefaultsQoS(), std::bind(&KinoReplanFSM::odometryCallback, this, std::placeholders::_1));
...
void KinoReplanFSM::checkCollisionCallback() {
  ...
  if (exec_state_ == FSM_EXEC_STATE::EXEC_TRAJ) {
    ...
    if (!safe) {
      ...
      RCLCPP_WARN(ros_node->get_logger(), "current traj in collision.");
      changeFSMExecState(REPLAN_TRAJ, "SAFETY");
    }
  }
}
