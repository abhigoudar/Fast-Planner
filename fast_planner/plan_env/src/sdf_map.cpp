...
void SDFMap::initMap(const rclcpp::Node::SharedPtr nh) {
  node_ = nh;

  /* get parameter */
  double x_size, y_size, z_size;
  assert(false && " loadParam initMap");
...
void SDFMap::updateOccupancyCallback() {
  ...
  md_.fuse_time_ += (t2.seconds() - t1.seconds());
  md_.max_fuse_time_ = max(md_.max_fuse_time_, (t2.seconds() - t1.seconds()));

  if (mp_.show_occ_time_)
    RCLCPP_WARN(node_->get_logger(),
      "Fusion: cur t = %lf, avg t = %lf, max t = %lf", (t2.seconds() - t1.seconds()),
...
void SDFMap::updateESDFCallback() {
  ...
  md_.esdf_time_ += (t2.seconds() - t1.seconds());
  md_.max_esdf_time_ = max(md_.max_esdf_time_, (t2.seconds() - t1.seconds()));

  if (mp_.show_esdf_time_)
    RCLCPP_WARN(node_->get_logger(),
      "ESDF: cur t = %lf, avg t = %lf, max t = %lf", (t2.seconds() - t1.seconds()),
